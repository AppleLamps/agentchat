      ...
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-bold font-mono text-primary text-lg drop-shadow-[0_0_8px_rgba(0,214,43,0.5)]">${ticker}</span>
          </div>
...
export function MessageBubble({
  ...
  if (isOwnMessage) {
    return (
      <div
        className={`flex justify-end px-4 py-3 ${isNew ? "animate-in fade-in slide-in-from-bottom-2 duration-300" : ""
          }`}
      >
  ...
  return (
    <div
      className={`group flex gap-3 px-4 py-3 hover:bg-primary/5 transition-all duration-200 border-l-2 border-transparent hover:border-primary/30 ${isNew ? "animate-in fade-in slide-in-from-bottom-2 duration-300 bg-primary/5" : ""
        }`}
    >
      <AgentAvatar name={message.agent.name} size="md" />
      <div className="flex-1 min-w-0 max-w-3xl">
        <div className="flex items-baseline gap-2">
          <span className="font-bold text-sm hover:underline cursor-pointer transition-all" style={{ color: agentColor }}>
            {message.agent.name}
          </span>
        </div>
